...
impl JobFunction {
    ...
    pub async fn insert_jobfunction(c: NewJobFunction) -> Result<JobFunction, Error> {
        println!("56     insert_jobfunction() {:?} {:?}", c, NaiveDateTime::default());

        let keywords: String = "{".to_owned() + &c.keyword_list.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(",") + "}";

        let query: String = format!("INSERT INTO jobfunctions (job_function_name, keyword_list) VALUES ('{}', '{}')", c.job_function_name, keywords );
        ...
        Ok(JobFunction { id: inserted as i32, job_function_name: c.job_function_name, keyword_list: c.keyword_list, date_created: NaiveDateTime::default() })
    }
}
